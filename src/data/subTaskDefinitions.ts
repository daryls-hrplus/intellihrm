...
export const subTaskDefinitions: Record<string, Record<number, SubTaskDefinition[]>> = {
  ...
  performance: {
    ...
    3: [ // Skills vs Competencies Foundation (includes Competency Library Integration)
      ...
      { order: 3, name: "Import or create competency library for the company", isRequired: true },
      { order: 4, name: "Filter to show COMPETENCY type only (not skills)", isRequired: true },
      { order: 5, name: "Generate AI-powered proficiency indicators for all 5 levels", isRequired: true },
      { order: 6, name: "Review and edit AI-generated behavioral indicators", isRequired: false },
      { order: 7, name: "Link competencies to jobs via job_capability_requirements", isRequired: true },
      { order: 8, name: "Set weight percentage for each competency-job link", isRequired: true },
      { order: 9, name: "Set required proficiency level (1-5) for each job", isRequired: true },
      { order: 10, name: "Validate competency weights equal 100% for each job", isRequired: true },
    ...
    8: [ // Appraisal Form Templates
      ...
      { order: 8, name: "Activate template for use in cycles", isRequired: true },
    ],
    ...
    10: [ // Employee Response Configuration
