...
export default function EnablementArtifactsPage() {
  ...
  const [tabState, setTabState] = useTabState({
    defaultState: {
      search: "",
      statusFilter: "all" as ArtifactStatus | "all",
      categoryFilter: "all",
      moduleFilter: "all",
    },
  });
  ...
  const filteredModules = useMemo(() => {
    ...
    if (tabState.categoryFilter === 'all') return childModules;
    const category = modules.find(m => m.id === tabState.categoryFilter);
    if (!category) return childModules;
    return childModules.filter(m => m.parent_module_code === category.module_code);
  }, [modules, tabState.categoryFilter]);

  // Reset module filter when category changes
  const handleCategoryChange = (value: string) => {
    setTabState({ categoryFilter: value, moduleFilter: 'all' });
  };

  const { artifacts, isLoading, fetchArtifacts } = useEnablementArtifacts({
    status: tabState.statusFilter === 'all' ? undefined : tabState.statusFilter as ArtifactStatus,
    moduleId: tabState.moduleFilter === 'all' ? undefined : tabState.moduleFilter,
  ...
  const handleArtifactClick = (artifact: typeof artifacts[0]) => {
    navigateToRecord({
      route: `/enablement/artifacts/${artifact.id}`,
  ...
  const handleCreateArtifact = () => {
    navigateToList({
      route: "/enablement/artifacts/new",
      title: "New Artifact",
      moduleCode: "enablement",
      icon: Plus,
    });
        ...
        <Card>
          <CardContent className="pt-4">
            <div className="flex flex-wrap gap-4">
              <div className="flex-1 min-w-[200px]">
                <div className="relative">
                  ...
                  <Input
                    ...
                    onChange={(e) => setTabState({ search: e.target.value })}
                    className="pl-9"
                  />
                </div>
              </div>
              <Select 
                value={tabState.statusFilter} 
                onValueChange={(v) => setTabState({ statusFilter: v as ArtifactStatus | 'all' })}
              >
              ...
              <Select value={tabState.categoryFilter} onValueChange={handleCategoryChange}>
              ...
              <Select 
                value={tabState.moduleFilter} 
                onValueChange={(v) => setTabState({ moduleFilter: v })}
              >
                <SelectTrigger className="w-[180px]">
