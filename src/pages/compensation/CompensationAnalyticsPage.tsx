...
export default function CompensationAnalyticsPage() {
  ...
  const { data: history = [], isLoading: historyLoading } = useQuery({
    ...
    queryFn: async () => {
      let query = supabase.from("compensation_history").select("*").order("effective_date", { ascending: true });
  ...
  const { data: bonusAwards = [], isLoading: bonusLoading } = useQuery({
    ...
    queryFn: async () => {
      let query = supabase.from("bonus_awards").select("*").eq("status", "paid");
      if (companyFilter !== "all") query = query.eq("company_id", companyFilter);
      const { data, error } = await query;
      if (error) throw error;
      return data || [];
  ...
  const { data: compaRatios = [], isLoading: compaLoading } = useQuery({
    ...
    queryFn: async () => {
      let query = supabase.from("compa_ratio_snapshots").select("*").order("snapshot_date", { ascending: false }).limit(100);
      if (companyFilter !== "all") query = query.eq("company_id", companyFilter);
      const { data, error } = await query;
      if (error) throw error;
      return data || [];
  ...
  const changeTypeData = history.reduce((acc: any[], h: any) => {
    ...
    if (existing) { existing.count += 1; existing.total += h.change_amount || 0; }
    else acc.push({ name: h.change_type, count: 1, total: h.change_amount || 0 });
    return acc;
  }, []);

  const bonusTypeData = bonusAwards.reduce((acc: any[], b: any) => {
    const existing = acc.find(a => a.name === b.bonus_type);
    if (existing) existing.value += b.final_amount || 0;
    else acc.push({ name: b.bonus_type, value: b.final_amount || 0 });
    return acc;
  }, []);
      ...
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold tracking-tight">{t("compensation.analytics.title")}</h1>
              <p className="text-muted-foreground">{t("compensation.analytics.subtitle")}</p>
            </div>
          </div>
          <div className="flex items-center gap-3">
        ...
        <div className="grid gap-4 sm:grid-cols-4">
          <Card><CardContent className="pt-6"><div className="flex items-center gap-4"><div className="rounded-lg bg-primary/10 p-3"><DollarSign className="h-5 w-5 text-primary" /></div><div><p className="text-sm text-muted-foreground">{t("compensation.analytics.totalSalaryChanges")}</p>{isLoading ? <Skeleton className="h-8 w-24" /> : <p className="text-2xl font-bold">${totalSalaryChanges.toLocaleString()}</p>}</div></div></CardContent></Card>
          <Card><CardContent className="pt-6"><div className="flex items-center gap-4"><div className="rounded-lg bg-emerald-500/10 p-3"><Award className="h-5 w-5 text-emerald-600" /></div><div><p className="text-sm text-muted-foreground">{t("compensation.analytics.bonusesPaid")}</p>{isLoading ? <Skeleton className="h-8 w-24" /> : <p className="text-2xl font-bold">${totalBonusPaid.toLocaleString()}</p>}</div></div></CardContent></Card>
          <Card><CardContent className="pt-6"><div className="flex items-center gap-4"><div className="rounded-lg bg-sky-500/10 p-3"><Target className="h-5 w-5 text-sky-600" /></div><div><p className="text-sm text-muted-foreground">{t("compensation.analytics.avgCompaRatio")}</p>{isLoading ? <Skeleton className="h-8 w-24" /> : <p className="text-2xl font-bold">{avgCompaRatio.toFixed(2)}</p>}</div></div></CardContent></Card>
