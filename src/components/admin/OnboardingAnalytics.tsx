...
interface AnalyticsData {
  ...
  tasksByType: { name: string; value: number; icon: string }[];
...
export function OnboardingAnalytics({ companyId }: OnboardingAnalyticsProps) {
  ...
  const loadAnalytics = async () => {
    ...
    try {
      ...
      // Fetch all tasks for these instances
      ...
      // Overdue instances (in_progress and past target date)
      const now = new Date();
      const overdue = instances?.filter(i => 
        i.status === 'in_progress' && 
        i.target_completion_date && 
        new Date(i.target_completion_date) < now
      ).length || 0;

      // Average completion time
      const completedInstances = instances?.filter(i => 
        i.status === 'completed' && i.actual_completion_date && i.start_date
      ) || [];
      const avgCompletionDays = completedInstances.length > 0
        ? completedInstances.reduce((sum, i) => 
            sum + differenceInDays(parseISO(i.actual_completion_date!), parseISO(i.start_date)), 0
          ) / completedInstances.length
        : 0;

      // On-time completion rate
      const onTimeCompleted = completedInstances.filter(i =>
        i.target_completion_date && 
        parseISO(i.actual_completion_date!) <= parseISO(i.target_completion_date)
      ).length;
      const onTimeCompletion = completedInstances.length > 0
        ? (onTimeCompleted / completedInstances.length) * 100
        : 0;
      ...
      instances?.forEach(instance => {
        ...
        if (instance.actual_completion_date) {
          const completedMonth = format(parseISO(instance.actual_completion_date), 'MMM yyyy');
      ...
      instances?.forEach(i => {
        const name = i.onboarding_templates?.name || 'Unknown';
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Card>
          <CardContent className="p-6">
            ...
            <Progress value={analytics.onTimeCompletion} className="mt-3 h-2" />
          </CardContent>
        </Card>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>New hires started vs completed over time</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="h-72">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Current onboarding status breakdown</CardDescription>
          </CardHeader>
          <CardContent>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Distribution of onboarding task types</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="h-64">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Onboarding templates by usage</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="space-y-4">
