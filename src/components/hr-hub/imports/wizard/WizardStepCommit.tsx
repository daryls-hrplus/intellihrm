...
import { transformJobFamiliesData, generateJobFamiliesFailureReport } from "./jobFamiliesTransformer";
import { transformJobsData, generateJobsFailureReport } from "./jobsTransformer";
...
export function WizardStepCommit({
  ...
  const downloadFailureReport = () => {
    ...
    if (importType === "positions") {
      ...
      report = generateDepartmentsFailureReport(importFailures, importWarnings);
    } else if (importType === "sections") {
      report = generateSectionsFailureReport(importFailures, importWarnings);
    } else if (importType === "job_families") {
  ...
  const handleCommit = async () => {
    ...
    try {
      ...
      if (importType === "companies") {
        setProgress(30);
        const transformResult = await transformCompaniesData(validData);
        ...
        setProgress(30);
        const transformResult = await transformDepartmentsData(validData);
        
        allErrors.push(...transformResult.errors);
        allWarnings.push(...transformResult.warnings.map(w => ({ 
          rowIndex: w.rowIndex, 
          field: "foreign_key", 
          message: w.message 
        })));
        ...
        setProgress(30);
        const transformResult = await transformSectionsData(validData);
        ...
        setProgress(30);
        const transformResult = await transformJobFamiliesData(validData);
        ...
        setProgress(30);
        const transformResult = await transformJobsData(validData);
        ...
        // The edge function auto-generates secure passwords for each user
        ...
        if (transformResult.transformed.length > 0) {
          ...
          // Passwords are auto-generated by the edge function
        ...
        <Card className="border-blue-200 bg-blue-50/50">
          <CardContent className="p-4">
            <div className="flex items-start gap-3">
              ...
              <div>
                <p className="font-medium text-blue-900">Secure Password Generation</p>
                <p className="text-sm text-blue-700">
                  Each imported employee will receive a unique, randomly generated secure password. 
                  They should use the "Forgot Password" feature to set their own password on first login.
                </p>
              </div>
            </div>
          </CardContent>
