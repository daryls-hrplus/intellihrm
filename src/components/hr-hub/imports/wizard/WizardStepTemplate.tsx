...
const TEMPLATE_CONFIGS: Record<string, { 
  ...
  companies: {
    headers: ["code", "name", "legal_name", "industry", "group_code", "division_code", "country", "currency", "address", "city", "state", "postal_code", "phone", "email", "website", "first_language", "second_language", "is_active"],
    fields: [
      ...
      { name: "legal_name", required: false, description: "Legal registered name (maps to name_en)", example: "Acme Corporation Ltd" },
      { name: "industry", required: false, description: "Industry sector", example: "Technology" },
      { name: "group_code", required: false, description: "Company group code (must exist)", example: "CORP" },
      { name: "division_code", required: false, description: "Division code (must exist)", example: "DIV001" },
      { name: "country", required: false, description: "Country code (ISO 2-letter)", example: "US", referenceDataType: "country" },
      { name: "currency", required: false, description: "Default currency code (maps to local_currency_id)", example: "USD", referenceDataType: "currency" },
      { name: "address", required: false, description: "Street address", example: "123 Main St" },
      { name: "city", required: false, description: "City", example: "New York" },
      { name: "state", required: false, description: "State/Province", example: "NY" },
      { name: "postal_code", required: false, description: "Postal/ZIP code", example: "10001" },
      { name: "phone", required: false, description: "Company phone number", example: "+1-555-1234" },
      { name: "email", required: false, description: "Company email address", example: "info@acme.com" },
      { name: "website", required: false, description: "Company website URL", example: "https://acme.com" },
      { name: "first_language", required: false, description: "Primary language (default: en)", example: "en", referenceDataType: "language" },
      { name: "second_language", required: false, description: "Secondary language", example: "es", referenceDataType: "language" },
      { name: "is_active", required: false, description: "Active status (true/false)", example: "true" },
    ],
    examples: [
      ["COMP001", "Acme Corp", "Acme Corporation Ltd", "Technology", "CORP", "", "US", "USD", "123 Main St", "New York", "NY", "10001", "+1-555-1234", "info@acme.com", "https://acme.com", "en", "", "true"],
      ["COMP002", "Beta Inc", "Beta Industries Inc", "Manufacturing", "CORP", "MFG", "GB", "GBP", "1 London Rd", "London", "", "SW1A 1AA", "+44-20-1234", "info@beta.com", "https://beta.com", "en", "", "true"],
    ],
    tips: [
      "Company codes must be unique across the system",
      "Currency codes (e.g., USD, TTD, JMD) are automatically resolved to local_currency_id",
      "Group and division codes must match existing records - check Reference Data for valid values",
      "Set is_active to 'true' or 'false' (defaults to true if omitted)",
      "Use ISO 2-letter country codes (e.g., US, GB, TT, JM)",
    ],
  },
  departments: {
    headers: ["code", "name", "company_code", "division_code", "description", "start_date", "end_date", "is_active"],
    fields: [
      { name: "code", required: true, description: "Unique department code within the company", example: "HR001" },
      { name: "name", required: true, description: "Department display name", example: "Human Resources" },
      { name: "company_code", required: true, description: "Parent company code (REQUIRED - must exist)", example: "CMG" },
      { name: "division_code", required: false, description: "Division code within the company (optional - if your company has divisions)", example: "OPS" },
      { name: "description", required: false, description: "Department description", example: "HR operations and talent management" },
    ...
    examples: [
      ["HR001", "Human Resources", "CMG", "", "HR operations and talent management", "2025-01-01", "", "true"],
      ["FIN001", "Finance", "CMG", "CORP", "Financial operations", "2025-01-01", "", "true"],
      ["PROD001", "Production", "CMG", "OPS", "Manufacturing production", "2025-01-01", "", "true"],
    ],
    tips: [
      "⚠️ Company code is REQUIRED - every department must belong to a company",
      "Division code is OPTIONAL - leave blank if your company doesn't use divisions",
      "If division_code is provided, it must exist within the specified company",
      "Import order: Companies → Divisions (if used) → Departments",
      "Use Reference Data drawer to view valid company and division codes",
    ],
  },
  sections: {
    headers: ["company_code", "department_code", "code", "name", "description", "start_date", "end_date"],
    fields: [
      { name: "company_code", required: true, description: "Company code (for scoping department lookup)", example: "COMP001" },
      { name: "department_code", required: true, description: "Parent department code (must exist within company)", example: "DEPT001" },
      { name: "code", required: true, description: "Unique section code within department", example: "SEC001" },
      { name: "name", required: true, description: "Section name", example: "Recruitment Section" },
      { name: "description", required: false, description: "Section description", example: "Handles recruitment" },
      { name: "start_date", required: true, description: "Start date (YYYY-MM-DD)", example: "2024-01-01" },
      { name: "end_date", required: false, description: "End date (YYYY-MM-DD, optional)", example: "" },
    ],
    examples: [
      ["COMP001", "DEPT001", "SEC001", "Recruitment Section", "Handles recruitment", "2024-01-01", ""],
      ["COMP001", "DEPT001", "SEC002", "Training Section", "Employee training", "2024-01-01", ""],
    ],
    tips: [
      "Company code and department code are REQUIRED - sections belong to departments",
      "Department must exist within the specified company before importing sections",
      "Import order: Companies → Departments → Sections",
      "Use Reference Data drawer to view valid company and department codes",
      "Section codes should be unique within each department",
    ],
  },
  divisions: {
  ...
  job_families: {
    headers: ["code", "name", "company_code", "master_code", "description", "start_date", "end_date", "is_active"],
    fields: [
      { name: "code", required: true, description: "Unique job family code within company", example: "TECH" },
      { name: "name", required: true, description: "Job family name", example: "Technology" },
      { name: "company_code", required: true, description: "Parent company code (must exist)", example: "CMG" },
      { name: "master_code", required: false, description: "Master job family code (optional - link to global standard)", example: "TECH" },
    ...
    examples: [
      ["TECH", "Technology", "CMG", "TECH", "All technology and IT roles", "2025-01-01", "", "true"],
      ["FIN", "Finance", "CMG", "FIN", "Finance and accounting roles", "2025-01-01", "", "true"],
      ["CUSTOM", "Custom Ops", "CMG", "", "Company-specific custom family", "2025-01-01", "", "true"],
    ],
    tips: [
      "Company code is REQUIRED - job families belong to companies",
      "Master code is OPTIONAL - use to link to global master job families for consistency",
      "If master_code is provided, your family inherits the global standard definition",
      "Leave master_code blank to create a fully custom job family",
      "Import companies BEFORE importing job families",
      "Use Reference Data drawer to see valid company codes and master job family codes",
    ],
  },
  jobs: {
  ...
  employee_assignments: {
    ...
    fields: [
      ...
      { name: "assignment_type", required: false, description: "Type of assignment", example: "primary", systemDefined: true, allowedValues: ["primary", "secondary", "acting", "temporary", "concurrent"] },
