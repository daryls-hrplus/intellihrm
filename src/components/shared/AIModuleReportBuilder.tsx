...
interface AIModuleReportBuilderProps {
...
export function AIModuleReportBuilder({ reportType, moduleName, companyId }: AIModuleReportBuilderProps) {
  ...
  useEffect(() => {
    loadReports();
  }, [companyId, reportType, moduleName]);
  ...
  const createReport = async () => {
    ...
    setIsCreating(true);
    try {
      const { data: analysisData, error: analysisError } = await supabase.functions.invoke('design-module-report', {
        body: {
  ...
  const iterateDesign = async () => {
    ...
    setIsLoading(true);
    try {
      const { data, error } = await supabase.functions.invoke('design-module-report', {
        body: {
        ...
        <Dialog open={showCreateDialog} onOpenChange={setShowCreateDialog}>
          ...
          <DialogContent className="max-w-lg">
            <DialogHeader>
              ...
              <DialogDescription>
                Upload a layout document to design your {moduleDisplayName} report with AI assistance
              </DialogDescription>
            </DialogHeader>
            <div className="space-y-4">
              <div>
              ...
              <div>
                ...
                <div className="mt-2 border-2 border-dashed rounded-lg p-6 text-center">
                  ...
                  <input
                    ...
                    className="hidden"
                    id={`layout-upload-${moduleName}-${reportType}`}
                  />
                  <label htmlFor={`layout-upload-${moduleName}-${reportType}`}>
