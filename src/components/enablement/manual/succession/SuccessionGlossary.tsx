...
import { SUCCESSION_GLOSSARY } from '@/types/successionManual';
import { Search, BookOpen } from 'lucide-react';

export function SuccessionGlossary() {
  const [searchTerm, setSearchTerm] = useState('');

  const categories = [...new Set(SUCCESSION_GLOSSARY.map(item => item.category))];
  
  const filteredTerms = SUCCESSION_GLOSSARY.filter(item =>
  ...
  const groupedTerms = categories.reduce((acc, category) => {
    ...
    return acc;
  }, {} as Record<string, typeof SUCCESSION_GLOSSARY>);
    ...
    <div className="space-y-8" id="glossary" data-manual-anchor="glossary">
      <div>
        ...
        <p className="text-muted-foreground mb-6">
          {SUCCESSION_GLOSSARY.length} terms and definitions for Succession Planning
        </p>
      </div>
