...
export async function generateProductCapabilitiesPdf(
  ...
  if (settings.sections.includeTableOfContents) {
    ...
    pdf.setTextColor(...secondaryRgb);
    pdf.setFont("helvetica", "bold");
    pdf.text("Table of Contents", margins.left, yPos);
    yPos += 15;
    
    // We'll render a placeholder TOC - in production you'd do a two-pass approach
    const renderTocEntry = (title: string, level: number, page: string) => {
      checkPageBreak(8);
      const indent = (level - 1) * 8;
      pdf.setFontSize(level === 1 ? 11 : 10);
      pdf.setFont("helvetica", level === 1 ? "bold" : "normal");
      pdf.setTextColor(level === 1 ? 30 : 71, level === 1 ? 41 : 85, level === 1 ? 59 : 105);
      ...
      pdf.setTextColor(100, 116, 139);
      pdf.text(page, pageWidth - margins.right, yPos, { align: 'right' });
      yPos += 6;
    };
    
    // Render TOC structure
    PRODUCT_CAPABILITIES_TOC.forEach((section, idx) => {
      renderTocEntry(section.title, 1, `${idx + 5}`);
      if (section.children && settings.sections.tocDepth >= 2) {
        section.children.forEach((child, childIdx) => {
          renderTocEntry(child.title, 2, `${idx + 5 + childIdx}`);
        });
      }
    });
  }
