...
function ModuleCardOverlay({ module }: { module: DraggableModule }) {
  const Icon = module.icon;
...
interface DraggableModuleGridProps {
  ...
  onReorder: (modules: DraggableModule[]) => void;
  onReset: () => void;
}

export function DraggableModuleGrid({ modules, onReorder, onReset }: DraggableModuleGridProps) {
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    if (over && active.id !== over.id) {
      const oldIndex = modules.findIndex((m) => m.tabCode === active.id);
      const newIndex = modules.findIndex((m) => m.tabCode === over.id);
      const newModules = arrayMove(modules, oldIndex, newIndex);
      onReorder(newModules);
    }
  };

  const activeModule = activeId ? modules.find((m) => m.tabCode === activeId) : null;
