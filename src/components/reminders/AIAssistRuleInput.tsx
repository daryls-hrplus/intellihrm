...
interface AIAssistRuleInputProps {
  companyId: string;
  onRuleCreated: () => void;
...
export function AIAssistRuleInput({ companyId, onRuleCreated, onSwitchToManual }: AIAssistRuleInputProps) {
  ...
  useEffect(() => {
    if (showReview && selectedEventTypeId && editedRule) {
  ...
  }, [showReview, selectedEventTypeId, editedRule?.daysBeforeIntervals, availableEventTypes, companyId]);
    ...
    return (
      <div className="space-y-4">
        <div className="text-center py-6">
          ...
          <h3 className="text-lg font-medium">Describe your rule in plain English</h3>
          <p className="text-sm text-muted-foreground">
            AI will parse your request and create the rule for you
          </p>
        </div>
        ...
        <div className="flex flex-wrap gap-2">
          {EXAMPLE_PROMPTS.map((prompt, index) => (
            <button
              ...
              className="text-xs px-3 py-1.5 rounded-full bg-muted hover:bg-primary/10 text-muted-foreground hover:text-foreground transition-colors"
            >
              {prompt}
            </button>
          ))}
