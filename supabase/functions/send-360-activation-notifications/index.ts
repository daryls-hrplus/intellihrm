...
serve(async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    console.log(`Processing 360 cycle activation notifications for cycle: ${cycleId}`);

    // Fetch cycle details
    const { data: cycle, error: cycleError } = await supabase
      .from("review_cycles")
      .select("name, company_id, start_date, end_date, self_review_deadline, feedback_deadline")
      .eq("id", cycleId)
      .single();
    ...
    // Fetch the 360 Cycle Activated email template
    const { data: template } = await supabase
      .from("reminder_email_templates")
      .select("subject, body")
      .eq("category", "performance")
      .eq("name", "360 Cycle Activated")
      .eq("is_active", true)
      .single();
    ...
    for (const participant of participants || []) {
      ...
      let subject = template?.subject || `360 Feedback Cycle Now Open: ${cycle.name}`;
      let body = template?.body || `Dear ${firstName},\n\nThe 360 feedback cycle "${cycle.name}" is now active.\n\nKey Dates:\n- Self-Review Deadline: ${formatDate(cycle.self_review_deadline)}\n- Feedback Deadline: ${formatDate(cycle.feedback_deadline)}\n- Cycle End Date: ${formatDate(cycle.end_date)}\n\nPlease log in to complete your assessments.\n\nBest regards,\n${company?.name || "HR"} Team`;
      ...
      body = body
        ...
        .replace(/\{\{self_review_deadline\}\}/g, formatDate(cycle.self_review_deadline))
        .replace(/\{\{feedback_deadline\}\}/g, formatDate(cycle.feedback_deadline))
        .replace(/\{\{end_date\}\}/g, formatDate(cycle.end_date));
      ...
      await supabase.from("notifications").insert({
        user_id: employee.id,
        title: "360 Feedback Cycle Started",
        message: `The 360 feedback cycle "${cycle.name}" is now active. Please complete your assessments.`,
        type: "360_cycle_start",
    ...
    console.error("Error in send-360-activation-notifications:", error);
    return new Response(
