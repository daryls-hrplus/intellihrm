...
-- Fix recalculate_leave_balance: ensure NOT NULL columns never receive NULL when no prior leave_balances row exists
...
BEGIN
  ...
  LOOP
    ...
    -- Get existing balance data if any; if none, keep defaults (0)
    SELECT 
    ...
    -- If no accrual rule found, use leave type default_annual_entitlement
    IF v_accrued_days = 0 AND v_leave_type.default_annual_entitlement IS NOT NULL THEN
      v_accrued_days := v_leave_type.default_annual_entitlement * (v_effective_months / 12.0);
    END IF;

    v_accrued_days := COALESCE(v_accrued_days, 0);
    v_taken_days := COALESCE(v_taken_days, 0);
    v_opening_balance := COALESCE(v_opening_balance, 0);
    v_adjustment_amount := COALESCE(v_adjustment_amount, 0);
    v_rollover_days := COALESCE(v_rollover_days, 0);
    ...
    -- Calculate new current balance (for display only, generated column will compute it)
