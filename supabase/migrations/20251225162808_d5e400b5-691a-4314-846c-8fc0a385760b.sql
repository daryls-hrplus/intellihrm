...
-- Fix recalculate_leave_balance function to use correct leave_types column names
-- The leave_types table has: default_annual_entitlement (not default_days), and no max_balance column
...
BEGIN
  ...
  FOR v_leave_type IN 
    SELECT id, name, default_annual_entitlement, max_consecutive_days
    FROM leave_types 
